...
#+hugo_base_dir: ~/Dropbox/website/blog/
...
(if (and (string-match-p "Windows" (getenv "PATH")) (not IS-WINDOWS))
    (setq dropbox-directory "/mnt/c/Users/usepackagex/Dropbox/")
...
(when (doom-font-exists-p "Iosevka Custom")
    (setq doom-font                (font-spec :name "CaskaydiaCoveNerdFontMono" :size 14)))
...
(when (doom-font-exists-p "CaskaydiaCoveNerdFontMono")
    (setq doom-symbol-font         (font-spec :name "CaskaydiaCoveNerdFontMono")))
#+end_src
...
#+begin_src emacs-lisp
(defun cup/pixel-recenter (&optional arg redisplay)
...
ARG and REDISPLAY are identical to the original function."
  ...
  (when-let* ((current-pixel (pixel-posn-y-at-point))
    ...
    (condition-case err
      ...
      (error (message "[cup/pixel-recenter] %s" (error-message-string err))))
    (when redisplay (redisplay t))))

(defun cup/pixel-scroll-up (&optional arg)
...
(defun cup/pixel-scroll-down (&optional arg)
          ...
          (lambda ()
            (cond
             (pixel-scroll-precision-mode
              (advice-add 'scroll-up :override 'cup/pixel-scroll-up)
              (advice-add 'scroll-down :override 'cup/pixel-scroll-down)
              (advice-add 'recenter :override 'cup/pixel-recenter))
             (t
              (advice-remove 'scroll-up 'cup/pixel-scroll-up)
              (advice-remove 'scroll-down 'cup/pixel-scroll-down)
              (advice-remove 'recenter 'cup/pixel-recenter)))))
#+end_src

* Editing configurations
** Handy functions
The =cup/fill-to-end= function function in Emacs Lisp fills the remaining space on the current line with a specified character until the line reaches the column defined by =fill-column=.
...
#+begin_src emacs-lisp
(defun cup/load-evil-keybindings ()
  (interactive)
  (load-file "~/.config/emacs/modules/config/default/+evil-bindings.el"))

(add-hook 'doom-after-init-hook #'cup/load-evil-keybindings)
#+end_src
...
(after! org-roam
  ;; Define advise
  (defun cup/org-roam-capf-add-kind-property (orig-fun &rest args)
  ...
  (advice-add 'org-roam-complete-link-at-point :around #'cup/org-roam-capf-add-kind-property)
  (advice-add 'org-roam-complete-everywhere :around #'cup/org-roam-capf-add-kind-property))

(after! citar
  ;; Define advise
  (defun cup/citar-capf-add-kind-property (orig-fun &rest args)
    "Advice around `org-roam-complete-link-at-point' to add :company-kind property."
    (let ((result (apply orig-fun args)))
      (append result '(:company-kind (lambda (_) 'reference)))))
  ;; Wraps around the relevant functions
  (advice-add 'citar-capf :around #'cup/citar-capf-add-kind-property))
#+end_src
...
(use-package! yasnippet
  ...
  (defun cup/change-cursor-color-if-yasnippet-can-fire (&optional field)
    (interactive)
  ...
  :hook (post-command . cup/change-cursor-color-if-yasnippet-can-fire))
#+end_src
...
** Workspaces
#+begin_src emacs-lisp
(defadvice! cup/config-in-its-own-workspace (&rest _)
...
(after! org
  ...
  (defun cup/org-mode-load-prettify-symbols ()
    (interactive)
  ...
  (when (not IS-WINDOWS)
    (add-hook 'org-mode-hook 'cup/org-mode-load-prettify-symbols)))
#+end_src
    ...
    (save-excursion
      ...
      (while (re-search-forward "^\\(\\#\\+begin_tips\\)" nil t)
        (let* ((beg (match-beginning 0))
          ...
          (overlay-put ov 'face 'cup/org-special-blocks-tips-face)))))

  (defun cup/org-add-overlay-info-blocks ()
      ...
      (while (re-search-forward "^\\(\\#\\+begin_\\(?:info\\|theorem\\)\\)" nil t)
        (let* ((beg (match-beginning 0))
               (end (if (re-search-forward "^\\(\\#\\+end_\\(?:info\\|theorem\\)\\)" nil t)
                      ...
                      (point-max)))
               (ov (make-overlay beg end)))
          (overlay-put ov 'face 'cup/org-special-blocks-info-face)))))

  (defun cup/org-add-overlay-warning-blocks ()
      ...
      (while (re-search-forward "^\\(\\#\\+begin_warning\\)" nil t)
        (let* ((beg (match-beginning 0))
               (end (if (re-search-forward "^\\(\\#\\+end_warning\\)" nil t)
                      ...
                      (point-max)))
               (ov (make-overlay beg end)))
          (overlay-put ov 'face 'cup/org-special-blocks-warning-face)))))

  (defun cup/org-add-overlay-note-blocks ()
      ...
      (while (re-search-forward "^\\(\\#\\+begin_\\(?:note\\|definition\\)\\)" nil t)
        (let* ((beg (match-beginning 0))
               (end (if (re-search-forward "^\\(\\#\\+end_\\(?:note\\|definition\\)\\)" nil t)
                      ...
                      (point-max)))
               (ov (make-overlay beg end)))
          (overlay-put ov 'face 'cup/org-special-blocks-note-face)))))

  (defun cup/org-add-overlay-question-blocks ()
      ...
      (while (re-search-forward "^\\(\\#\\+begin_\\(?:question\\|proposition\\)\\)" nil t)
        (let* ((beg (match-beginning 0))
               (end (if (re-search-forward "^\\(\\#\\+end_\\(?:question\\|proposition\\)\\)" nil t)
                      ...
                      (point-max)))
               (ov (make-overlay beg end)))
          (overlay-put ov 'face 'cup/org-special-blocks-question-face)))))


  (add-hook! '(org-mode-hook yas-after-exit-snippet-hook)
             '(cup/org-add-overlay-tips-blocks
               cup/org-add-overlay-info-blocks
               cup/org-add-overlay-warning-blocks
               cup/org-add-overlay-note-blocks
               cup/org-add-overlay-question-blocks)))
#+end_src
...
(use-package! org-agenda
  ...
  ;; Todo/checkbox statistic to count nested headings
...
(use-package! org-super-agenda
  ...
  (setq org-agenda-custom-commands
        '(("o" "Overview"
           ((agenda "")
            ...
            ;;         '((:auto-map cup/agenda-auto-group-title-olp)))))
  ...
  (defun cup/agenda-auto-group-title-olp (item)
...
(use-package! org-capture
  ...
  (defun cup/org-capture-maybe-create-id ()
    (when (org-capture-get :create-id)
      (org-id-get-create)))
  (add-hook 'org-capture-mode-hook #'cup/org-capture-maybe-create-id)
  ;;Auxiliary functions
  (defun cup/capture-ox-hugo-post (lang)
    (setq cup/ox-hugo-post--title (read-from-minibuffer "Post Title: ")
          cup/ox-hugo-post--fname (org-hugo-slug cup/ox-hugo-post--title)
          cup/ox-hugo-post--fdate (format-time-string "%Y-%m-%d"))
    (expand-file-name (format "%s_%s.%s.org" cup/ox-hugo-post--fdate cup/ox-hugo-post--fname lang)
  ...
  (setq org-capture-templates
        `(("i" "Inbox" entry (file ,(concat org-directory "/Agenda/inbox.org"))
          ...
          ("b" "New blog post")
          ("be" "English" plain (file (lambda () (cup/capture-ox-hugo-post "en")))
           ,(string-join
             '("#+title: %(eval cup/ox-hugo-post--title)"
               "#+subtitle:"
               "#+author: %n"
               "#+filetags: blog"
               "#+date: %(eval cup/ox-hugo-post--fdate)"
               "#+hugo_base_dir: ~/Dropbox/Blogs/hieutkt/"
               "#+hugo_section: ./posts/"
               "#+hugo_tags: %?"
               "#+hugo_url: ./%(eval cup/ox-hugo-post--fname)"
               "#+hugo_slug: %(eval cup/ox-hugo-post--fname)"
               "#+hugo_custom_front_matter:"
               "#+hugo_draft: false"
               "#+startup: content"
               "#+options: toc:2 num:t")
           ...
           :immediate-finish t
           :jump-to-captured t)
          ("bv" "Vietnamese" plain (file (lambda () (cup/capture-ox-hugo-post "vi")))
           ,(string-join
             '("#+title: %(eval cup/ox-hugo-post--title)"
               "#+subtitle:"
               "#+author: %n"
...
(use-package! oc
  :config
  (setq org-cite-csl-styles-dir (concat dropbox-directory "research/zotero/styles/")
...
(use-package! org-roam
  ...
  (setq org-roam-completion-everywhere nil
        ...
        cup/org-roam-function-tags '("compilation" "argument" "journal" "concept" "tool" "data" "bio" "literature" "event" "website"))
  :config
  ;; Org-roam interface
  ...
  (cl-defmethod org-roam-node-othertags ((node org-roam-node))
    ...
    (let* ((tags (seq-filter (lambda (tag) (not (string= tag "ATTACH"))) (org-roam-node-tags node)))
           (specialtags cup/org-roam-function-tags)
  ...
  (setq org-roam-capture-templates
        `(("d" "default" plain "%?"
           ...
           (file+head "${slug}_%<%Y-%m-%d--%H-%M-%S>.org"
                      "#+title: ${title}\n#+created: %U\n#+filetags: %(completing-read \"Function tags: \" cup/org-roam-function-tags)\n#+startup: overview")
           :unnarrowed t))))
...
#+begin_src emacs-lisp
(defface cup/org-roam-count-overlay-face
  '((t :inherit org-list-dt :height 0.8))
  "Face for Org Roam count overlay.")

(defun cup/org-roam--count-overlay-make (pos count)
  (let* ((overlay-value (propertize
                         (concat "·" (format "%d" count) " ")
                         'face 'cup/org-roam-count-overlay-face 'display '(raise 0.2)))
...
(defun cup/org-roam--count-overlay-remove-all ()
  (dolist (ov (overlays-in (point-min) (point-max)))
    (when (overlay-get ov 'roam-backlinks-count)
      (delete-overlay ov))))

(defun cup/org-roam--count-overlay-make-all ()
  (cup/org-roam--count-overlay-remove-all)
        ...
        (let* ((id (org-element-property :path elem))
               (count (caar
                       (org-roam-db-query
                        ...
                        id))))
          (when (< 0 count)
            (cup/org-roam--count-overlay-make
             (org-element-property :end elem)
             count)))))))

(define-minor-mode cup/org-roam-count-overlay-mode
  "Display backlink count for org-roam links."
  :after-hook
  (if cup/org-roam-count-overlay-mode
      (progn
        (cup/org-roam--count-overlay-make-all)
        (add-hook 'after-save-hook #'cup/org-roam--count-overlay-make-all nil t))
    (cup/org-roam--count-overlay-remove-all)
    (remove-hook 'after-save-hook #'cup/org-roam--count-overlay-remove-all t)))

(add-hook 'org-mode-hook #'cup/org-roam-count-overlay-mode)
#+end_src
...
#+begin_src emacs-lisp
(defun cup/delete-empty-org-files (directory)
...
#+begin_src emacs-lisp
(defun cup/org-roam-get-previous-dailies-file ()
...
(defun cup/org-roam-migrate-todos (&rest _)
  "Migrate TODOs from the previous day's Org-roam file to today's file."
  (interactive)
  (let ((yesterday-file (cup/org-roam-get-previous-dailies-file))
    ...
    (when (and yesterday-file (file-exists-p yesterday-file))
      (with-current-buffer (find-file-noselect yesterday-file)
        ...
        (cup/delete-empty-org-files (file-name-directory yesterday-file))
...
#+begin_src emacs-lisp :tangle no
(advice-add 'org-roam-dailies-goto-today :after #'cup/org-roam-migrate-todos)
#+end_src
...
#+begin_src emacs-lisp
(defun cup/org-insert-timestamped-bullet ()
...
(define-key org-mode-map (kbd "C-c t") 'cup/org-insert-timestamped-bullet)
#+end_src

#+RESULTS:
: cup/org-insert-timestamped-bullet
*** Org-download
...
(use-package! elfeed
  ...
  :bind ("<f10>" . #'elfeed)
  :config
  ;; (defun cup/elfeed-entry-line-draw (entry)
  ...
  (defadvice! cup/mark-read (&rest _)
  ...
  (defface cup/elfeed-blog
    `((t :foreground ,(doom-color 'blue)))
    "Marks a Elfeed blog.")
  (push '(blog cup/elfeed-blog)
        elfeed-search-face-alist)
  (push '(read elfeed-search-title-face)
        elfeed-search-face-alist)

  ;; Variables
  (setq elfeed-search-print-entry-function 'cup/elfeed-entry-line-draw
...
#+begin_src emacs-lisp
(after! (elfeed)
  (defadvice! cup/elfeed-in-own-workspace (&rest _)
  "Open Elfeeds in its own workspace."
  :before #'elfeed
  (when (modulep! :ui workspaces)
